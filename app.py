...
<div class="dropdown">
  ...
  <ul class="dropdown-menu dropdown-menu-end" style="background:var(--card);border:1px solid var(--border);min-width:220px">
    ...
    <li><a class="dropdown-item" href="/export/word" style="color:var(--text)">📝 Word Report</a></li>
    <li><hr class="dropdown-divider"></li>
    <li><a class="dropdown-item" href="/change_password" style="color:var(--text)">🔑 Change Password</a></li>
    <li><a class="dropdown-item" href="/health" style="color:var(--text)">✅ Health Check</a></li>
    <li><hr class="dropdown-divider"></li>
...
def dashboard():
    ...
    html+="<div class='row g-2 mt-3'>"
    html+="<div class='col-12 col-md-6'><div class='card-dark' style='border:1px solid #fbbf24;background:linear-gradient(135deg,#1e293b,#0f172a)'><h6 style='color:#fbbf24'>🏆 Top 3 OT Earners - Ranking UI</h6><div class='mt-3'>"
    ...
    html+="</div></div></div>"
    html+="<div class='col-12 col-md-6'><div class='card-dark' style='border:1px solid #8b5cf6;background:linear-gradient(135deg,#1e1b4b,#0f172a)'><h6 style='color:#8b5cf6'>⭐ Top 3 QA - Ranking UI</h6><div class='mt-3'>"
    if top_qa:
...
def view(aid):
    ...
    html+=f"<div class='row g-2 mt-3'><div class='col-4'><div class='kpi' style='border:1px solid #22c55e'><div class='label'>NORMAL</div><div class='val-big' style='color:#22c55e'>{n}h</div></div></div><div class='col-4'><div class='kpi' style='border:1px solid #3b82f6'><div class='label'>RESTDAY</div><div class='val-big' style='color:#3b82f6'>{r}h</div></div></div><div class='col-4'><div class='kpi' style='border:1px solid #ef4444'><div class='label'>LOSS</div><div class='val-big' style='color:#ef4444'>{loss}h</div></div></div><div class='col-6'><div class='kpi' style='border:1px solid #22c55e'><div class='label'>TOTAL OT</div><div class='val-big' style='color:#22c55e'>{tot}h</div></div></div><div class='col-6'><div class='kpi' style='border:1px solid #fbbf24'><div class='label'>NET</div><div class='val-big' style='color:#fbbf24'>+{net}h</div></div></div><div class='col-6'><div class='kpi' style='border:1px solid #f97316;min-height:80px;height:80px'><div class='label'>AHT</div><div class='val-big' style='font-size:16px;color:#f97316'>{la}m</div></div></div><div class='col-6'><div class='kpi' style='border:1px solid #8b5cf6;min-height:80px;height:80px'><div class='label'>QA</div><div class='val-big' style='font-size:16px;color:#8b5cf6'>{lq}%</div></div></div></div>"
    ...
    if not is_agent:
      ...
      """
        ...
        html+=f"<div class='card-dark mt-3'><h6 style='color:#94a3b8'>View Only - Agent Mode</h6><small style='color:#64748b'>Risk {risk}</small></div>"
...
def export_page():
    ...
    <div class='row g-3 mt-3'>
      ...
      <div class='col-12 col-md-6'><div class='card-dark' style='border:1px solid #8b5cf6'><h6 style='color:#8b5cf6'>Export Word - Executive Report</h6><a href='/export/word' class='btn w-100' style='background:#8b5cf6;color:white'>📝 Download Word</a></div></div>
      <div class='col-12 col-md-6'><div class='card-dark' style='border:1px solid #fbbf24'><h6 style='color:#fbbf24'>Export PDF - Executive Report</h6><a href='/export/pdf' class='btn w-100' style='background:#fbbf24;color:#0b1120'>📄 Download PDF</a></div></div>
...
def export_csv():
    ...
    writer.writerow(["ID","NAME","TENCENT_ID","TOTAL_OT","LOSS","NET","AHT","QA","WH_TARGET","WH_COMP"])
    for a in agents:
        logs=get_ot(a.get("id"))
        n,r,tot,loss,net=calc(logs)
        plogs=get_perf(a.get("id"))
        ...
        wh_target=float(a.get("WORKING_HOURS_TARGET",220))
        wh_comp=((wh_target-loss)/wh_target*100) if wh_target>0 else 0
        writer.writerow([a.get("id"),a.get("NAME"),a.get("TENCENT_ID"),tot,loss,net,la,lq,wh_target,round(wh_comp,1)])
...
def export_excel():
    try:
        ...
        try:
            ...
            # --- Colors (matches the app's Blue Ice & White / dark theme) ---
            NAVY="0B1120"; GOLD="FBBF24"; WHITE_TXT="FFFFFF"
            BAND="F1F5F9"; GREEN="16A34A"; AMBER="D97706"; RED="DC2626"

            header_font=Font(bold=True, color=WHITE_TXT, size=11)
            header_fill=PatternFill("solid", fgColor=NAVY)
            title_font=Font(bold=True, color=WHITE_TXT, size=16)
            title_fill=PatternFill("solid", fgColor=NAVY)
            band_fill=PatternFill("solid", fgColor=BAND)
            comment_font=Font(italic=True, size=10, color="334155")
            ...
            def band_rows(ws, start_row, end_row, ncols):
            ...
            # --- Fetch once, group locally (avoids N+1 Firebase round-trips) ---
            all_ot=get_all_ot()
            all_perf=get_all_perf()
            ot_by_agent=_dd(list); perf_by_agent=_dd(list)
            for l in all_ot: ot_by_agent[str(l.get("agent_id"))].append(l)
            for l in all_perf: perf_by_agent[str(l.get("agent_id"))].append(l)
            ...
            for a in agents:
                ...
                agent_stats.append({"id":a.get("id"),"name":a.get("NAME",""),"tid":a.get("TENCENT_ID",""),
                    "tot":tot,"loss":loss,"net":net,"aht":la,"qa":lq,
            ...
            at_risk=[a for a in agent_stats if a["loss"]>=4]

            # --- Executive comment, dynamically generated from attendance/QA/AHT ---
            att_word = "strong and on target" if overall_att>=95 else "acceptable but worth monitoring" if overall_att>=90 else "a concern that needs attention"
            qa_word = "no QA data has been logged yet" if avg_qa==0 else "performing well above target" if avg_qa>=85 else "within an acceptable range" if avg_qa>=75 else "below target and should be reviewed"
            aht_word = "no AHT data has been logged yet" if avg_aht==0 else "efficient" if avg_aht<=7 else "slightly above the ideal handling window" if avg_aht<=10 else "elevated and may be impacting overall throughput"
            exec_comment = (
                f"Team attendance for this period is {round(overall_att,1)}%, which is {att_word}. "
                f"Average QA score stands at {avg_qa}%, {qa_word}. "
                f"Average handling time (AHT) is {avg_aht} minutes, which is {aht_word}. "
                f"{len(at_risk)} agent(s) are currently flagged for loss hours of 4 or more and may need follow-up coaching."
            )
            ...
            ws["A1"]="TEAM SHINE M9 — EXECUTIVE PERFORMANCE REPORT"
            ...
            ws["A2"]=f"Generated {datetime.now(PH_TZ).strftime('%B %d, %Y — %I:%M %p')}  |  Team size: {len(agents)} agents"
            ws["A2"].font=Font(italic=True, size=9, color="64748B")
            ws.row_dimensions[2].height=16

            ws.merge_cells("A4:G4")
            ws["A4"]="Executive Comment"
            ws["A4"].font=Font(bold=True, size=12, color="0B1120")
            ws.merge_cells("A5:G8")
            ws["A5"]=exec_comment
            ws["A5"].font=comment_font
            ws["A5"].alignment=Alignment(wrap_text=True, vertical="top", horizontal="left")
            ws.row_dimensions[5].height=70

            r=10
            ws.cell(row=r, column=1, value="KPI Summary").font=Font(bold=True, size=12, color="0B1120")
            r+=1
            header_row=r
            for c, h in enumerate(["Metric","Value","Unit","Status"], start=1):
                ws.cell(row=r, column=c, value=h)
            style_header_row(ws, r, 4)
            r+=1
            kpi_start=r
            kpi_data=[
                ("Team Avg QA", avg_qa, "%", "Good" if avg_qa>=85 else "Watch" if avg_qa>=75 else "Attention"),
                ("Team Avg AHT", avg_aht, "minutes", "—"),
                ("Team Attendance", round(overall_att,1), "%", "Good" if overall_att>=95 else "Watch" if overall_att>=90 else "Attention"),
                ("Total Normal OT", round(team_n,1), "h", "—"),
                ("Total Restday OT", round(team_r,1), "h", "—"),
                ("Total OT", round(team_tot,1), "h", "—"),
                ("Total Loss", round(team_loss,1), "h", "—"),
                ("Team Net", round(team_tot-team_loss,1), "h", "—"),
                ("Agents at Risk (Loss ≥4h)", len(at_risk), "agents", "Attention" if at_risk else "Good"),
            ]
            for row in kpi_data:
                ws.append(row)
            for i in range(len(kpi_data)):
                row_idx=kpi_start+i
                status=kpi_data[i][3]
                sc = GREEN if status=="Good" else RED if status=="Attention" else AMBER if status=="Watch" else "94A3B8"
                cell=ws.cell(row=row_idx, column=4)
                cell.font=Font(bold=True, color=sc)
            band_rows(ws, kpi_start, kpi_start+len(kpi_data)-1, 4)
            r=kpi_start+len(kpi_data)+2

            ws.cell(row=r, column=1, value="Agent Detail").font=Font(bold=True, size=12, color="0B1120")
            r+=1
            header_row2=r
            headers2=["ID","NAME","TENCENT_ID","TOTAL_OT","LOSS","NET","AHT","QA","WH_TARGET","WH_ACTUAL","WH_COMP%"]
            ...
            for a in sorted(agent_stats, key=lambda x:-x["wh_comp"]):
                ws.append([a["id"],a["name"],a["tid"],round(a["tot"],1),round(a["loss"],1),round(a["net"],1),
                           round(a["aht"],1) if a["aht"] else 0, round(a["qa"],1) if a["qa"] else 0,
                           a["wh_target"], round(a["wh_actual"],1), round(a["wh_comp"],1)])
            detail_end=detail_start+len(agent_stats)-1
            band_rows(ws, detail_start, detail_end, len(headers2))
            for i, a in enumerate(sorted(agent_stats, key=lambda x:-x["wh_comp"])):
                if a["loss"]>=4:
                    ws.cell(row=detail_start+i, column=5).font=Font(bold=True, color=RED)

            autosize(ws, [8,20,12,10,10,10,9,8,11,11,11])
            ws.freeze_panes="A11"

            # --- OT Logs sheet (executive styling, no gridlines, banded, no N+1) ---
            agent_lookup={str(a.get("id")): (a.get("NAME",""), a.get("TENCENT_ID","")) for a in agents}
            ws2=wb.create_sheet("OT Logs")
            ws2.sheet_view.showGridLines=False
            ws2.append(["Agent ID","Name","Tencent ID","Date","Type","Hours","Reason"])
            style_header_row(ws2, 1, 7)
            for l in all_ot:
                aname, tid = agent_lookup.get(str(l.get("agent_id")), ("",""))
                ws2.append([l.get("agent_id"),aname,tid,l.get("date"),l.get("type"),l.get("hours"),l.get("reason")])
            if all_ot:
                band_rows(ws2, 2, len(all_ot)+1, 7)
            autosize(ws2, [10,20,12,12,14,10,30])

            # --- QA/AHT Logs sheet (executive styling, no gridlines, banded, no N+1) ---
            ws3=wb.create_sheet("QA AHT Logs")
            ...
            if all_perf:
                band_rows(ws3, 2, len(all_perf)+1, 7)
            autosize(ws3, [10,20,12,12,10,10,30])

            from io import BytesIO
            bio=BytesIO()
            wb.save(bio)
            bio.seek(0)
            return Response(bio.getvalue(), mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", headers={"Content-Disposition":"attachment;filename=Team_Shine_M9_Executive_Report.xlsx"})
        except ImportError:
            output=io.StringIO()
            import csv
            writer=csv.writer(output)
            ...
            all_ot=get_all_ot(); all_perf=get_all_perf()
            ot_by_agent=defaultdict(list); perf_by_agent=defaultdict(list)
            for l in all_ot: ot_by_agent[str(l.get("agent_id"))].append(l)
            for l in all_perf: perf_by_agent[str(l.get("agent_id"))].append(l)
            agent_stats=[]
            team_n=0; team_r=0; team_loss=0; team_tot=0
            for a in agents:
                ...
                plogs=perf_by_agent.get(str(a.get("id")), [])
                la,lq,lc,lf,aa,qa,ac,af=calc_perf(plogs)
                agent_stats.append({"qa":lq,"aht":la,"wh_target":float(a.get("WORKING_HOURS_TARGET",220))})
            team_qa=[a["qa"] for a in agent_stats if a["qa"]>0]
            team_aht=[a["aht"] for a in agent_stats if a["aht"]>0]
...
# Wrapped defensively: a problem in this file should never take the whole
# app down at startup — worst case, the coaching feature is just unavailable.
try:
    import coaching
except Exception as e:
    print(f"[coaching] failed to load, coaching feature disabled: {e}")
# --- end Coaching Minutes integration ---

# --- Executive PDF report integration (added) ---
# Loads pdf_export.py, which swaps in a fixed, more executive /export/pdf.
# Wrapped defensively: if reportlab/matplotlib aren't available in this
# environment (or any other error occurs), the app still boots and
# /export/pdf simply falls back to whatever export_pdf app.py already
# defines above, instead of crashing the whole deployment.
try:
    import pdf_export
except Exception as e:
    print(f"[pdf_export] failed to load, falling back to original /export/pdf: {e}")
# --- end Executive PDF report integration ---

# --- Executive Word report integration (added) ---
# Loads docx_export.py, which adds a brand-new /export/word route.
# Wrapped defensively so a missing/broken dependency (python-docx) can't
# take the whole app down — worst case, the Word export just isn't available.
try:
    import docx_export
except Exception as e:
    print(f"[docx_export] failed to load, Word export disabled: {e}")
# --- end Executive Word report integration ---

# --- Bug fix (moved) ---
# This block was previously in the MIDDLE of the file (right after
# export_excel and before export_pdf), which meant that if this app is ever
# started with `python app.py` directly, execution would block at app.run()
# and every route defined afterward — export_pdf, coaching, pdf_export —
# would silently never be registered. Moved here, to the true end of the
# file, so all routes are always registered regardless of how the app is
# started. This has no effect if you start the app with gunicorn (the usual
    ...
    try:
        ...
        try:
            ...
            for a in agents:
                ...
                plogs=get_perf(a.get("id"))
                la,lq,lc,lf,aa,qa,ac,af=calc_perf(plogs)
                agent_stats.append({"name":a.get("NAME",""),"tid":a.get("TENCENT_ID",""),"tot":tot,"loss":loss,"qa":lq,"aht":la,"wh_target":float(a.get("WORKING_HOURS_TARGET",220))})
            team_qa=[a["qa"] for a in agent_stats if a["qa"]>0]
            team_aht=[a["aht"] for a in agent_stats if a["aht"]>0]
            total_wh=sum([a["wh_target"] for a in agent_stats])
            total_actual=total_wh-team_loss
            ...
            ws.append(["ID","NAME","TENCENT_ID","TOTAL_OT","LOSS","NET","AHT","QA","WH_TARGET","WH_ACTUAL","WH_COMP%"])
            for a in agents:
                logs=get_ot(a.get("id"))
                n,r,tot,loss,net=calc(logs)
                plogs=get_perf(a.get("id"))
                ...
                wh_actual=wh_target-loss
                wh_comp=(wh_actual/wh_target*100) if wh_target>0 else 0
                ws.append([a.get("id"),a.get("NAME"),a.get("TENCENT_ID"),tot,loss,net,la,lq,wh_target,round(wh_actual,1),round(wh_comp,1)])
            ws2=wb.create_sheet("OT Logs")
            ws2.append(["Agent ID","Name","Tencent ID","Date","Type","Hours","Reason"])
            all_ot=get_all_ot()
            for l in all_ot:
