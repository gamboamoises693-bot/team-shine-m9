...
def get_db():
    ...
    if not firebase_admin._apps:
        url = os.environ.get("FIREBASE_DATABASE_URL") or os.environ.get("FIREBASE_DB_URL")
        if not url:
            raise Exception("FIREBASE_DATABASE_URL not set in Environment Variables")
        if os.path.exists("serviceAccountKey.json"):
            ...
            raise Exception("No serviceAccountKey.json and no FIREBASE_CREDENTIALS found")
...
@app.route("/")
def index():
    url = os.environ.get("FIREBASE_DATABASE_URL","NOT SET")
    has_file = os.path.exists("serviceAccountKey.json")
    has_env = bool(os.environ.get("FIREBASE_CREDENTIALS"))
    return f"""
    <h2>SAFE MODE LIVE</h2>
    DB_URL: {url[:60]}<br>
    Has serviceAccountKey.json file: {has_file}<br>
    Has FIREBASE_CREDENTIALS env: {has_env}<br>
    <a href='/test'>Test Firebase Connect</a><br>
    <a href='/migrate_now'>Migrate Now (safe keys)</a><br>
    <a href='/check'>Check Count</a><br>
    <a href='/ping'>Ping</a>
    """
...
def test():
    try:
        db_root = get_db()
        db_root.child("test").set({"ok": "connected"})
        return f"Firebase CONNECTED OK! <a href='/migrate_now'>Migrate now</a>"
    except Exception as e:
        return f"<pre>CONNECT FAILED: {e}\n{traceback.format_exc()}</pre>"
...
def check():
    try:
        ...
        return f"Has {len(data)} agents<br><a href='/'>Home</a>"
    except Exception as e:
...
def migrate_now():
    try:
        ...
        for ag in OLD_AGENTS:
            ...
            count+=1
        data = db_root.child("agents").get() or {}
        return f"<h1>DONE! Migrated {count}, now total {len(data)} agents!</h1><a href='/check'>Check</a><br>Now upload FINAL app"
    except Exception as e:
        return f"<pre>MIGRATE FAILED: {e}\n{traceback.format_exc()}</pre>"
