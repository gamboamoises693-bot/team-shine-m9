...
@app.route("/")
def index():
    return f"Mode: {FIREBASE_MODE} Error: {err} <br><a href='/migrate_now'>Tap to Migrate 19 agents</a> | <a href='/check'>Check count</a>"

@app.route("/check")
def check():
    try:
        data = db_root.child("agents").get() or {}
        return f"Realtime has {len(data)} agents<br>{'<br>'.join([str(v.get('NAME','')) for v in data.values()][:20])}<br><a href='/'>Home</a>"
    except Exception as e:
        return f"Error: {e}"
...
def migrate_now():
    try:
        existing = db_root.child("agents").get() or {}
        if len(existing) >= 5:
            return f"Already {len(existing)} agents - <a href='/check'>Check</a> | <a href='/clear'>Clear first if want re-migrate</a>"
        count=0
        for ag in OLD_AGENTS:
            ...
            count+=1
        return f"<h1>DONE! Migrated {count} agents to Realtime!</h1><a href='/check'>Check</a> - Now upload real app"
    except Exception as e:
        import traceback
...
def clear():
    try:
        ...
        return "Cleared - <a href='/migrate_now'>Migrate again</a>"
    except Exception as e:
        return str(e)
